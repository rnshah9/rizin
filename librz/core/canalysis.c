...
static RzList *core_load_all_signatures_from_sigdb(RzCore *core, bool with_details) {
	...
	const char *prefix = rz_config_get(core->config, "dir.prefix");
	char *prefix_sigdb = rz_file_path_join(prefix, RZ_SIGDB);
	if (RZ_STR_ISNOTEMPTY(prefix_sigdb) && rz_file_is_directory(prefix_sigdb)) {
		sysdb = rz_sign_sigdb_load_database(prefix_sigdb, with_details);
	}
	free(prefix_sigdb);
